...
def _detect_impact_level(summary: str, changed_files: list) -> ImpactLevel:
    ...
    if any(keyword in summary_lower for keyword in ["critical", "hotfix", "security"]): return ImpactLevel.CRITICAL
    if any(keyword in summary_lower for keyword in ["major", "breaking", "api"]): return ImpactLevel.HIGH
    if any(keyword in summary_lower for keyword in ["typo", "docs", "readme"]): return ImpactLevel.LOW
    if len(changed_files) > 10: return ImpactLevel.HIGH
    if len(changed_files) <= 2: return ImpactLevel.LOW
...
def update_changelog(project_root: Optional[Path] = None):
    ...
    if gemini_client and gemini_client.is_ready():
        ...
        ai_summary = gemini_client.generate_summary(text_prompt=prompt, changed_files=changed_files)
        summary = ai_summary
    ...
    if new_version != old_version:
        print(f"   📈 Version update determined: {old_version} → {new_version} ({increment_type})")
...
def _create_initial_project_entry(json_manager, project_root: Path):
    ...
    json_manager.add_entry(ai_summary=welcome_summary, changed_files=[], impact_level=ImpactLevel.LOW, change_type=ChangeType.CONFIG)
