...
class VersionManager:
    ...
    def auto_increment_based_on_branch(self) -> Tuple[str, str, str]:
        ...
        if not branch_name:
            logger.warning(
                "Could not determine git branch. Falling back to default patch increment."
            )
        ...
        logger.info(
            f"Branch-based increment determined: '{increment_type}'. New version: {new_version}"
        )
    ...
    def create_git_tag(self, version: str, codename: Optional[str] = None) -> bool:
        ...
        try:
            ...
            result = self._run_git_command(
                ["tag", "-a", tag_name, "-m", message], capture_output=True
            )

            if result:
                logger.info(
                    f"Git tag {tag_name} created successfully with message: '{message}'"
                )
                return True
            else:
                return False
    ...
    def update_version_in_files(self, new_version: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully updated version in package.json to {new_version}"
            )
            return True

        except Exception as e:
            logger.error(
                f"An unexpected error occurred while updating version in files: {e}"
            )
            return False
        ...
        try:
            ...
            if output:
                ...
                if match:
                    ...
                    logger.info(
                        f"Found codename '{codename}' in git tag for version {version}."
                    )
                    return codename
        except Exception as e:
    ...
    def _get_version_codename(
        self, major: int, minor: int, new_version: str, gemini_client=None
    ) -> str:
        """
...
def auto_version_management(
    project_root: Path, changed_files: list, impact_level: str, ai_summary: str
) -> Optional[str]:
    ...
    try:
        ...
        if new_version_str != current_version_str:
            ...
            print(
                f"   ✅ Version unchanged or update failed for v{current_version_str}"
            )
