...
class GitManager:
    ...
    def _run_external_command(self, command: List[str]) -> Optional[str]:
        ...
        try:
            ...
            tool = command[0]
            logger.error(
                f"Command '{tool}' not found. Please ensure it is installed and in your system's PATH."
            )
            return None
    ...
    def get_open_issues(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            return None
        issues_json = self._run_external_command(
    ...
    def get_diff(self) -> Optional[str]:
        ...
        return self._run_git_command(["diff", "--staged"]) or self._run_git_command(
            ["diff"]
        )
