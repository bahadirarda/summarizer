...
def main():
    ...
    # A more robust check might parse the output for actual errors.
    # For now, we just run it to show the report. A real CI would fail on score.
    ...
    # You might want to use a more robust cleaning method
    subprocess.run(['rm', '-rf', str(project_root / 'dist')], check=False)
    
    build_return_code, _ = run_command([sys.executable, '-m', 'build'], cwd=project_root)
    if build_return_code != 0:
        print("❌ Build failed. Aborting release.")
        sys.exit(1)
    
    # Find the created artifacts
