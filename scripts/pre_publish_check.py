...
def test_api_key():
    ...
    try:
        ...
        if not gemini_client.is_ready():
            return print_result(
                False, "Gemini client is not ready - API key missing or invalid"
            )

        # Basit bir test çağrısı yap
        test_response = gemini_client.generate_summary(
            "Test message for API validation"
        )
        ...
        if test_response and "AI özeti alınamadı" in test_response:
            ...
            test_response
            and len(test_response) > 20
            and "test" in test_response.lower()
        ):
            return print_result(True, "API key is valid and working")
        else:
            return print_result(
                False, f"API key test failed - unexpected response: {test_response}"
            )
...
def validate_version_consistency():
    ...
    try:
        ...
        if package_version == vm_version:
            ...
            return print_result(
                False,
                f"Version mismatch! package.json: v{package_version}, Version Manager: v{vm_version}",
            )
...
def check_git_status():
    ...
    try:
        ...
        if result.stdout.strip():
            return print_result(
                False,
                "Uncommitted changes found! Please commit all changes before publishing.",
            )

        # Remote ile sync mi?
        result = subprocess.run(
            ["git", "status", "-uno"], capture_output=True, text=True, cwd=project_root
        )

        if "ahead" in result.stdout:
            return print_result(
                False,
                "Local commits not pushed! Please push to remote before publishing.",
            )
