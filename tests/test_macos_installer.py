...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'macos-setup-wizard', 'src'))
...
try:
    ...
    pytest.skip(f"macOS installer components not available: {e}", allow_module_level=True)
...
class TestCLIInstaller:
    ...
    def test_cli_installer_user_mode(self, mock_input):
        ...
        with patch.object(installer, '_create_directories') as mock_dirs, \
             patch.object(installer, '_copy_files') as mock_copy, \
             patch.object(installer, '_update_shell_profile') as mock_shell:
