...
from src.utils.json_changelog_manager import ImpactLevel, ChangeType
...
def get_changelog():
    ...
    try:
        ...
        # Validate enums
        change_type_enum = validate_enum_parameter(change_type, ChangeType, 'change type')
        impact_level_enum = validate_enum_parameter(impact_level, ImpactLevel, 'impact level')
        ...
        if search:
            ...
            entries = [
                ...
                if (search_lower in entry.ai_summary.lower() or
                    any(search_lower in file.lower() for file in entry.changed_files) or
                    any(search_lower in tag.lower() for tag in entry.tags))
            ]
...
def get_stats():
    ...
    try:
        ...
        # Calculate trends
        recent_entries = [e for e in entries if (datetime.now() - datetime.fromisoformat(e.timestamp)).days <= 7]
...
def get_timeline():
    ...
    try:
        ...
        for entry in entries:
            ...
            timeline[key]["stats"]["by_type"][change_type] = timeline[key]["stats"]["by_type"].get(change_type, 0) + 1
            timeline[key]["stats"]["by_impact"][impact_level] = timeline[key]["stats"]["by_impact"].get(impact_level, 0) + 1
