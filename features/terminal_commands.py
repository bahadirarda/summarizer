    ...
    try:
        ...
        if sys.platform == "win32":  # Windows
            ...
            if not installed:
                ...
                print(
                    "⚠️  To use 'summarizer' globally, add this directory to your PATH:"
                )
                        ...
                        if install_dir == Path("/usr/local/bin"):
                            ...
                            try:
                                ...
                                with open(temp_script, "w") as f:
                                    f.write(script_content)
                                temp_script.chmod(
                                    temp_script.stat().st_mode | stat.S_IEXEC
                                )
            ...
            if not installed:
                ...
                print(f"📁 Created script: {script_path}")
                print(
                    "⚠️  To use 'summarizer' globally, add this to your shell profile:"
                )
...
def check_terminal_command():
    ...
    try:
        result = subprocess.run(
            [script_name, "--help"], capture_output=True, text=True, timeout=5
        )
