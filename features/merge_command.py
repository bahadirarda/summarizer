...
from src.utils.io import _ask_user
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if mergeable_data.get('mergeable') != 'CONFLICTING':
                ...
                # Try merging via GitHub CLI
                print("   🚀 Executing merge via GitHub CLI...")
                merge_success = git_manager.merge_pull_request(pr_to_merge['number'], pr_to_merge['baseRefName'])
                
                if merge_success:
                    print(f"   ✅ PR #{pr_to_merge['number']} successfully merged!")
                    return MergeStatus.SUCCESS
                else:
                    print(f"   ❌ Failed to merge PR #{pr_to_merge['number']}")
                    if attempt < max_retries:
                        # Offer conflict resolution
                        if _ask_user("   ❔ Would you like to try automatic conflict resolution?"):
                            if git_manager.resolve_conflicts_with_pr(pr_to_merge['number']):
                                print("   ✅ Conflicts resolved! Retrying merge...")
                                continue
                            else:
                                print("   ❌ Automatic conflict resolution failed.")
                    
                    if attempt == max_retries:
                        # Final attempt options
                        print("\n   ⚠️  All merge attempts failed.")
                        print("   💡 Options:")
                        print("      1. 'force' - Force push with lease (CAUTION)")
                        print("      2. 'manual' - Switch to manual conflict resolution")
                        print("      3. 'cancel' - Cancel merge operation")
                        
                        choice = input("   ❔ Choose an option (force/manual/cancel): ").lower().strip()
                        
                        if choice == 'force':
                            current_branch = git_manager.get_current_branch()
                            if git_manager.force_push_with_confirmation(current_branch):
                                print("   ✅ Force push completed successfully! Retrying merge...")
                                # Try merge one more time after force push
                                final_merge_success = git_manager.merge_pull_request(pr_to_merge['number'], pr_to_merge['baseRefName'])
                                return MergeStatus.SUCCESS if final_merge_success else MergeStatus.FAILED
                            else:
                            ...
                            print("      1. Check out the PR branch locally")
                            print("      2. Merge the target branch")
                            print("      3. Resolve conflicts manually")
                            print("      4. Push and retry merge")
                            return MergeStatus.CANCELLED
            ...
            if _ask_user(prompt):
                ...
                print("      1. Force Merge: Overwrite the target branch to resolve conflicts (DANGEROUS).")
                print("      2. Cancel: I will resolve the conflict manually.")
                
                while True:
                    choice = input("   Enter your choice (1 or 2): ").strip()
                    if choice == '1':
                        if git_manager.force_push_with_confirmation(source_branch, target_branch=target_branch):
                            print("\n   🎉 Force-merge completed. The PR can now be merged (or closed).")
                            return MergeStatus.SUCCESS # The action succeeded, so we return success.
                        else:
                            print("   ❌ Force-merge was cancelled or failed.")
                            return MergeStatus.CANCELLED
                        break
                    elif choice == '2':
...
- **Issue Relations:** Check if PR title mentions "Closes #X" or "Fix #X" - these are important for issue tracking.
...
def merge_command(project_root: Path):
    ...
    try:
        ...
        if not config_manager.get_api_key():
            ...
            if _ask_user("   ❔ Run setup now to add it?"):
                setup_command()
        ...
        if config_manager.get_api_key():
            ...
            if gemini_client.is_ready():
                ...
                if _ask_user("   ❔ Would you like to update your GEMINI_API_KEY?"):
                    try:
                        print("   🔧 Launching API key setup...")
                        setup_command()
                        print("   ✅ Setup complete. Please restart the merge command to use the new key.")
                        return  # Exit to allow restart with new key
                    except Exception as setup_error:
                        print(f"   ❌ Setup failed: {setup_error}")
                        print("   ⚪️ Continuing without AI assistant.")
                else:
                    print("   ⚪️ Continuing without AI assistant.")
        else:
        ...
        if "api" in str(e).lower() or "key" in str(e).lower() or "auth" in str(e).lower():
            if _ask_user("   ❔ This might be an API key issue. Would you like to update your GEMINI_API_KEY?"):
                try:
                    print("   🔧 Launching API key setup...")
                    setup_command()
                    print("   ✅ Setup complete. Please restart the merge command to use the new key.")
    ...
    if merge_status == MergeStatus.SUCCESS:
        ...
        try:
            ...
            if git_manager.get_uncommitted_changes():
                print("   - Stashing local changes to prevent checkout errors...")
                stash_success, _ = git_manager.stash_changes()
                if stash_success:
                    stashed_changes = True
                    print("   - Changes stashed successfully.")
                else:
                    print("   - ⚠️  Failed to stash changes. Skipping checkout.")
                    return

            # Checkout and pull the target branch
            target_branch = pr_to_merge.get('baseRefName', 'main')
            git_manager.checkout(target_branch)
            git_manager.pull(target_branch)

            # Ask before deleting the local source branch
            source_branch = pr_to_merge.get('headRefName')
            if _ask_user(f"   ❔ Do you want to delete the local branch '{source_branch}'?"):
                git_manager.delete_local_branch(source_branch)
                print(f"   - Deleted local branch '{source_branch}'.")
