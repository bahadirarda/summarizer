...
from src.utils.changelog_updater import _ask_user
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if mergeable_data.get('mergeable') != 'CONFLICTING':
                ...
                success, output = git_manager.merge_pull_request(pr_to_merge['number'], pr_to_merge['baseRefName'])
                if success:
                    print(f"   ✅ Successfully merged PR #{pr_to_merge['number']} using {merge_method} method")
...
"""
    try:
        ...
        # Check if response indicates quota exceeded or error
        if not response_text or "429" in response_text or "quota" in response_text.lower() or "rate limit" in response_text.lower() or "error" in response_text.lower():
            print("   ⚠️  AI quota exceeded. Your daily limit has been reached.")
            if _ask_user("   ❔ Would you like to update your GEMINI_API_KEY?"):
                ...
                print("   ⚪️ Continuing without AI recommendations.")
            return None
        ...
        if json_match:
            try:
                ...
                print("   ⚠️  AI response contains invalid JSON. Continuing without AI recommendations.")
                return None
        else:
        ...
        if "429" in str(e) or "quota" in str(e).lower():
            ...
            print("   ⚠️  AI analysis temporarily unavailable. Continuing without AI recommendations.")
            # For other API errors, also offer to update the key
